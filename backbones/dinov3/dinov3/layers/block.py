...
class SelfAttentionBlock(nn.Module):
    ...
    def _forward(self, x: Tensor, rope=None, return_attn=False) -> Tensor:
        """
        ...
        """
        # If returning attention, skip residual connections and just return attention
        if return_attn:
            attn_result = self.attn(self.norm1(x), rope=rope, return_attn=True)
    ...
    def _forward_list(self, x_list: List[Tensor], rope_list=None, return_attn=False) -> List[Tensor]:
        """
        ...
        """
        # If returning attention, not supported for list inputs
        if return_attn:
            raise NotImplementedError("return_attn not supported for list inputs")
    ...
    def forward(self, x_or_x_list, rope_or_rope_list=None, return_attn=False) -> List[Tensor]:
        if isinstance(x_or_x_list, Tensor):
            # When return_attn=True, use _forward directly to avoid list wrapping issue
            if return_attn:
                return self._forward(x_or_x_list, rope=rope_or_rope_list, return_attn=True)
            # for reference:
            # return self._forward(x_or_x_list, rope=rope_or_rope_list, return_attn=return_attn)
            # in order to match implementations we call the list op:
            return self._forward_list([x_or_x_list], rope_list=[rope_or_rope_list], return_attn=return_attn)[0]
        elif isinstance(x_or_x_list, list):
            if rope_or_rope_list is None:
                rope_or_rope_list = [None for x in x_or_x_list]
            # return [self._forward(x, rope=rope, return_attn=return_attn) for x, rope in zip(x_or_x_list, rope_or_rope_list)]
            return self._forward_list(x_or_x_list, rope_list=rope_or_rope_list, return_attn=return_attn)
        else:
