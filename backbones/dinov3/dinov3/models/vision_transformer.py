...
class DinoVisionTransformer(nn.Module):
    ...
    def get_last_self_attention(self, x: Tensor) -> Tensor:
        """Get self-attention from last transformer layer.
        ...
        for blk in self.blocks[:-1]:
            x = blk(x, rope_sincos, return_attn=False)

        # Last block: extract attention
        return self.blocks[-1](x, rope_sincos, return_attn=True)

    def get_all_self_attentions(self, x: Tensor) -> List[Tensor]:
        ...
        for blk in self.blocks:
            ...
            attn = blk(x, rope_sincos, return_attn=True)
            attentions.append(attn)

            # Also run normal forward for next layer
            x = blk(x, rope_sincos, return_attn=False)
